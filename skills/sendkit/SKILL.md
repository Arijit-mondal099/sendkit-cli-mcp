...
SendKit sends Telegram messages. It exposes the same operation two ways, both backed by `@sendkit-cli-mcp/core`:

- **MCP tool** (`sendkit` server -> `telegram` tool) - preferred for agents.
- **CLI** (`@sendkit-cli-mcp/cli`, binary `sendkit`) - fallback when MCP is unavailable or for manual verification.
...
Run the CLI without a global install via `bunx @sendkit-cli-mcp/cli telegram <chatId> <message>` (or the `npx` equivalent).
